...
from ..model_utils import DepthNet, DepthNet2
...
class LSSViewTransformerBEVDepth(LSSViewTransformer):
    ...
    def forward(self, input, stereo_metas=None):
        ...
        ## 获取深度监督点
        # x, depth_supervision = self.depth_net(x, mlp_input, stereo_metas)
        ...
        # return bev_feat, depth, depth_supervision
    ...
    # @force_fp32()
    # def get_depth_loss_2(self, depth_labels, depth_preds, depth_supervision):
    #     """
    #     修正深度监督损失 - 使用离散分布损失
    #     """
    #     # 原始深度损失
    #     main_loss = self.get_depth_loss(depth_labels, depth_preds)
    #
    #     # 深度监督点损失 - 使用离散分布损失
    #     # 将深度监督点视为深度概率分布
    #     depth_supervision = depth_supervision.softmax(dim=1)  # (B*N, D, fH, fW)
    #
    #     # 获取下采样的深度标签
    #     depth_labels_down = self.get_downsampled_gt_depth(depth_labels)
    #
    #     # 计算监督点损失（与主损失相同）
    #     depth_supervision = depth_supervision.permute(0, 2, 3, 1).contiguous().view(-1, self.D)
    #     fg_mask = torch.max(depth_labels_down, dim=1).values > 0.0
    #     depth_labels_down = depth_labels_down[fg_mask]
    #     depth_supervision = depth_supervision[fg_mask]
    #
    #     with autocast(enabled=False):
    #         supervision_loss = F.binary_cross_entropy(
    #             depth_supervision,
    #             depth_labels_down,
    #             reduction='none',
    #         ).sum() / max(1.0, fg_mask.sum())
    #
    #     # 组合损失
    #     return main_loss + 0.5 * supervision_loss

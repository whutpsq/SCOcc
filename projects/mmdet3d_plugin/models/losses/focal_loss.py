...
class CustomFocalLoss(nn.Module):
    ...
    def forward(self,
        ...
        device = pred.device  # 获取当前设备

        # 将c_base转移到当前设备并扩展
        c = self.c_base.to(device)[None, :, :, None].repeat(B, 1, 1, D).reshape(-1)
        ...
        # 使用索引前确保所有张量在同一设备
        c_visible = c[visible_mask, None]
        if weight is not None:
            weight_mask = weight[None, :] * c_visible
        else:
            weight_mask = c_visible  # 如果weight为None，只使用c
